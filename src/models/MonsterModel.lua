...
function MonsterModel:new(type, x, y)
    local self = setmetatable({}, MonsterModel)
    
    -- 设置唯一ID
    self.id = nextMonsterId
    ...
    -- 其他属性
    self.loot = self.config.loot or {}
    self.expValue = self.config.expValue or 10
...
function MonsterModel:setTarget(target)
    if target then
        if type(target) == "table" and target.getPosition then
            -- 如果是玩家控制器或其他对象，获取其位置
            local pos = target:getPosition()
            self.target = {x = pos.x, y = pos.y, getPosition = target.getPosition}
        else
            -- 直接设置目标
            self.target = target
        end
...
function MonsterModel:updateAI(dt, map)
    ...
    -- 计算与目标的距离
    local dx = targetPos.x - self.x
    local dy = targetPos.y - self.y
    ...
    if distance <= self.attributes.detectRange then
        self.ai.state = "chase"
        self.ai.lastSeenTarget = {x = targetPos.x, y = targetPos.y}
